...
import { useEditSummaryMutation, useGetSummaryQuery } from '../api/careerSummaryApi';
import { logout } from '../auth/authSlice';

const EditCareerSummary = () => {
    const navigate = useNavigate();
    const goBack = () => navigate(-1);
    const dispatch = useDispatch()
    const { userId } = useParams()
    
    const {data: summary, isError, error } = useGetSummaryQuery(userId);
    
    const [formData, setFormData] = useState({
      careerSummary: summary?.CareerSummary
    })
  
    const [editSummary, { isSuccess, isError: isEditError, error: editError, isLoading }] = useEditSummaryMutation();
    ...
    useEffect(() => {
        if(isEditError){
            if(editError?.status === 401){
                dispatch(logout())
                toast.error('Please re-authenticate to continue')
                navigate('/login', { replace: true })
            } else if(editError?.status === 403){
                navigate('/unauthorized', { replace: true })
            } else {
                toast.error(editError?.data?.Message)
            }
        }
    }, [navigate, editError, isEditError, dispatch])
...
export default EditCareerSummary